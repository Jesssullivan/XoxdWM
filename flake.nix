{
  description = "XoxdWM: native Wayland WM/DE authority with Emacs/eGreg app-layer clients";
        ...
        let
          ...
          mkCompositor = { pname, features ? [ ], extraBuildInputs ? [ ] }:
            pkgs.rustPlatform.buildRustPackage {
              ...
              meta = with pkgs.lib; {
                description = "XoxdWM Wayland compositor built on Smithay";
        ...
        builtins.foldl' pkgs.lib.recursiveUpdate { } [
          {
            devShells.default = pkgs.mkShell {
              ...
              '' + ''
                echo "XoxdWM dev shell ready"
                echo "  rustc: $(rustc --version)"
                echo "  emacs: $(emacs --version | head -1)"
                echo ""
                echo "  cachix: use 'cachix use exwm-vr' to enable the compatibility binary cache"
              '';
            };
        ...
        packages.aarch64-linux =
          ...
          in
          {
            compositor = pkgs.rustPlatform.buildRustPackage {
              ...
              meta = with pkgs.lib; {
                description = "XoxdWM Wayland compositor built on Smithay (aarch64)";
                license = licenses.gpl3Plus;
            ...
            compositor-headless = pkgs.rustPlatform.buildRustPackage {
              ...
              meta = with pkgs.lib; {
                description = "XoxdWM Wayland compositor headless (aarch64)";
                license = licenses.gpl3Plus;
                platforms = [ "aarch64-linux" ];
              };
            };
        ...
        packages.s390x-linux =
          ...
          in
          {
            compositor-headless = pkgs.rustPlatform.buildRustPackage {
              ...
              meta = with pkgs.lib; {
                description = "XoxdWM Wayland compositor headless (s390x)";
                license = licenses.gpl3Plus;
