...
| `honey` VR session | Smoke | On `2026-04-22`, the installed `ewwm-compositor` on `honey` initialized `wp_drm_lease_v1`, reserved `DP-2` via `~/.config/exwm-vr/compositor.env`, and granted a real DRM lease to Monado. A repo-owned `exwm-vr-monado.service` then proved three runtime surfaces on the host: the older local `/usr/local/bin/monado-service` lane, a staged `monado-beyond` companion RPM tree from run `24804821792`, and an installed `monado-beyond` host package from run `24807084915` using `/usr/bin/monado-service` with no `MONADO_SERVICE_BIN` override. The repo now has `just honey-openxr-status` and `just honey-openxr-smoke` wrappers for the OpenXR client lane, and the `exwm-vr-openxr-smoke-client` RPM from run `24938791255` is installed on `honey`; status preflight now resolves `/usr/libexec/exwm-vr/hello_xr -g Vulkan`. This is still not a repeatable operator lane because the packaged client has only reached status/preflight and the full VR run has only been captured once; see [Honey Substrate Proof](docs/honey-substrate-proof-2026-04-22.md). |
