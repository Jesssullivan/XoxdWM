...
| `honey` VR session | Smoke | On `2026-04-22`, the installed `ewwm-compositor` on `honey` initialized `wp_drm_lease_v1`, reserved `DP-2` via `~/.config/exwm-vr/compositor.env`, and granted a real DRM lease to Monado. A repo-owned `exwm-vr-monado.service` plus `~/.config/exwm-vr/monado.env` also reached a live direct-mode `hello_xr -g Vulkan` run without service drop-ins. This is still not a repeatable zero-override operator lane because `honey` still points that env file at a local `/usr/local/bin/monado-service` build; see [Honey Substrate Proof](docs/honey-substrate-proof-2026-04-22.md). |
