...
| `honey` VR session | Smoke | On `2026-04-22`, the installed `ewwm-compositor` on `honey` initialized `wp_drm_lease_v1`, reserved `DP-2` via an explicit host-side lease override, granted a real DRM lease to Monado, and `hello_xr -g Vulkan` reached `READY` plus eye swapchain creation without the old IPC shim. This is still not a repeatable zero-override operator lane; see [Honey Substrate Proof](docs/honey-substrate-proof-2026-04-22.md). |
