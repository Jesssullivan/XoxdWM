...
| `honey` VR session | Smoke | On `2026-04-22`, the installed `ewwm-compositor` on `honey` initialized `wp_drm_lease_v1`, reserved `DP-2` via `~/.config/exwm-vr/compositor.env`, and granted a real DRM lease to Monado. A repo-owned `exwm-vr-monado.service` then proved two runtime surfaces on the host: the older local `/usr/local/bin/monado-service` lane and a staged `monado-beyond` companion RPM tree from run `24804821792`, which reached direct-mode client connect plus eye swapchain creation. This is still not a repeatable zero-override operator lane because `monado-beyond` is not yet installed on-host, stale Monado IPC sockets from the older local lane still need cleanup, and the current local `hello_xr` build still needs the literal IPC shim; see [Honey Substrate Proof](docs/honey-substrate-proof-2026-04-22.md). |
