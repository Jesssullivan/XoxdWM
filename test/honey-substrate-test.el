...
(ert-deftest honey-substrate/honey-openxr-status-uses-plain-read-only-ssh ()
  ...
  (with-temp-buffer
    ...
    (let* ((justfile (buffer-string))
      ...
      (should (string-match-p (regexp-quote "EXWM_VR_VISUAL_CONFIRMATION") stanza))
      (should (string-match-p (regexp-quote "bash -s -- --status-only") stanza))
      (should (string-match-p (regexp-quote "< \"{{project_root}}/packaging/scripts/exwm-vr-openxr-smoke\"") stanza))
      (should-not (string-match-p (regexp-quote "cd \"{{remote_repo_path}}\"") stanza))
      (should-not (string-match-p (regexp-quote "./packaging/scripts/exwm-vr-openxr-smoke --status-only") stanza))
      (should-not (string-match-p (regexp-quote "just honey-run") stanza))
      (should-not (string-match-p (regexp-quote "nix develop") stanza)))))

(ert-deftest honey-substrate/honey-openxr-smoke-streams-local-wrapper ()
  "The active OpenXR lane should not trust a stale remote checkout wrapper."
  (with-temp-buffer
    (insert-file-contents honey-substrate--justfile)
    (let* ((justfile (buffer-string))
           (start (string-match "^honey-openxr-smoke host=\"honey\"" justfile))
           (end (and start (string-match "^\\[group" justfile (1+ start))))
           (stanza (and start end (substring justfile start end))))
      (should stanza)
      (should (string-match-p (regexp-quote "EXWM_VR_VISUAL_OBSERVED") stanza))
      (should (string-match-p (regexp-quote "EXWM_VR_VISUAL_OBSERVER") stanza))
      (should (string-match-p (regexp-quote "EXWM_VR_VISUAL_CONFIRMATION") stanza))
      (should (string-match-p (regexp-quote "bash -s --") stanza))
      (should (string-match-p (regexp-quote "< \"{{project_root}}/packaging/scripts/exwm-vr-openxr-smoke\"") stanza))
