...
- The latest native Rocky RPM blocker is no longer the compositor build itself. The repair lane now treats both SELinux hardening and the BrainFlow BCI virtualenv as separate opt-in package paths so the base Rocky compositor RPM can be proven first.
