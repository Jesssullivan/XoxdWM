...
- `yoga` still does not have a fully proved local login/session path. The host now has the installed `exwm-vr.desktop` session entry, user-unit surface, and an active SDDM greeter on `seat0`, so the remaining gap is no longer "missing display manager"; it is a physical `EXWM-VR` session selection and local-login proof. The originally observed `exwm-vr-emacs.service` stop-path failure is now understood, and the in-tree `SuccessExitStatus=15` fix has been validated on-host via a temporary user override; the next packaged RPM needs to carry that fix.
