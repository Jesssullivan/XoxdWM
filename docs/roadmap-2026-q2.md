...
- `honey` now has a direct-mode proof from the installed compositor package surface, a staged `monado-beyond` companion RPM proof from run `24804821792`, and an installed-host `monado-beyond` follow-up from run `24807084915` that reaches active `/usr/bin/monado-service` plus eye swapchain creation. The `exwm-vr-openxr-smoke-client` RPM from run `24938791255` is installed on `honey`; on `2026-04-25` EDT, three bounded packaged-client smoke passes reached Monado / Bigscreen Beyond and created eye swapchains in one active service session. The remaining gap is turning that into a fresh-boot or clean-teardown-cycle operator lane with explicit stale-socket handling.
