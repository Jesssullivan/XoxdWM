...
- `honey` now has a direct-mode proof from the installed compositor package surface, a staged `monado-beyond` companion RPM proof from run `24804821792`, and an installed-host `monado-beyond` follow-up from run `24807084915` that reaches active `/usr/bin/monado-service` plus eye swapchain creation. The `exwm-vr-openxr-smoke-client` RPM from run `24938791255` is installed on `honey`; on `2026-04-25` EDT, three bounded packaged-client smoke passes and three clean stop/start cycles reached Monado / Bigscreen Beyond and created eye swapchains. The remaining gap is turning that into a fresh-boot operator lane and then into in-goggles first-frame / longer-running stability evidence.
