...
| `honey` VR session | Design | `hello_xr` now reaches Monado runtime selection, Bigscreen Beyond selection, and Vulkan device creation on `honey` in the current forced Wayland-window fallback. A follow-up direct Wayland probe fails because the compositor does not expose DRM lease support to Monado, and the fallback path still crashes at `xrCreateSession` with `VK_ERROR_SURFACE_LOST_KHR`. This is a concrete substrate blocker, not a working VR smoke path yet. |
...
| DRM backend on AMD | Smoke | Code exists and bounded named-host startup is now recorded on both `yoga` and `honey`; full `honey` VR smoke remains pending because the Monado session path still crashes. |
| VR session lifecycle | Design | Not yet claimed as working on `honey`. |
| DRM lease / HMD non-desktop handling | Design | The repo has HMD connector bookkeeping, but the live `honey` direct-mode probe still reports `Compositor is missing drm-lease support`, and the current smoke path still brings `DP-2` up as a normal output. |
