...
| `yoga` XoxdWM install | Smoke | `yoga` now has installed `0.5.4-1.el10` `exwm-vr-*` RPMs from Actions run `24768509226`, and the real installed units pass a named-host proof with explicit `drm` backend and dedicated Emacs bootstrap. The repo-side `SuccessExitStatus=15` stop-path fix is also validated on-host via temporary override. The remaining local-login gap is structural: the host has the installed `exwm-vr.desktop` session entry, but no display-manager package or `display-manager.service`; see [yoga-session-proof-2026-04-22.md](yoga-session-proof-2026-04-22.md). |
