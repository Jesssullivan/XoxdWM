...
| OpenXR smoke-client Rocky RPM | Smoke | GitHub Actions run `24938791255` produced `exwm-vr-openxr-smoke-client-0.0.1-1.20260425git.el10.x86_64.rpm`; the package is installed on `honey`, provides `/usr/bin/exwm-vr-hello-xr -> ../libexec/exwm-vr/hello_xr`, and `just honey-openxr-status` now resolves `/usr/libexec/exwm-vr/hello_xr -g Vulkan`. This is client provenance and preflight proof, not a repeated runtime proof by itself. |
