...
| OpenXR smoke-client Rocky RPM | Smoke | GitHub Actions run `24938791255` produced `exwm-vr-openxr-smoke-client-0.0.1-1.20260425git.el10.x86_64.rpm`; the package is installed on `honey`, provides `/usr/bin/exwm-vr-hello-xr -> ../libexec/exwm-vr/hello_xr`, and three bounded runs on `2026-04-25` EDT selected `/usr/libexec/exwm-vr/hello_xr -g Vulkan` successfully. This proves packaged-client runtime smoke within one active service session, not fresh-boot or clean-teardown-cycle stability by itself. |
...
| DRM backend on AMD | Smoke | Code exists and bounded named-host startup is now recorded on both `yoga` and `honey`; `honey` now also has direct-mode lease proof with Monado plus `hello_xr`, including three packaged-client smoke passes in one active session. It is not yet a fresh-boot or clean-teardown-cycle operator lane. |
| VR session lifecycle | Smoke | `honey` now has a direct-mode session proof where `hello_xr -g Vulkan` reached `READY` and created eye swapchains from the installed compositor package surface. It also has a staged Monado companion RPM proof that reached active direct-mode Monado plus eye swapchain creation. The repo-owned OpenXR smoke wrapper plus installed smoke-client package now give immediate repeated smoke evidence in one active service session, but the packaged-client path is not yet claimed as a stable first-frame, long-running, fresh-boot, or teardown-cycle proof. |
