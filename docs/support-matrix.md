...
| Monado companion Rocky RPM | Smoke | GitHub Actions run `24804821792` produced `monado-beyond-0.0.1-1.20260310git.el10.x86_64.rpm`. A staged non-root proof on `honey` reached active direct-mode Monado plus eye swapchain creation after staging `hidapi`, clearing a stale `monado_comp_ipc` socket, and restoring the current literal `hello_xr` IPC shim. This is not yet the installed host lane. |
