...
| `honey` XoxdWM compositor install | Smoke | `honey` now has branch-scoped `exwm-vr-0.5.4-1.el10` packages installed, and `systemctl --user start exwm-vr.target` reached a bounded named-host startup with active compositor plus Emacs, `DP-2` at `5088x2544@75Hz`, `HDMI-A-2` at `1920x1080@60Hz`, and `ewwm: initialized`; see [honey-substrate-proof-2026-04-22.md](honey-substrate-proof-2026-04-22.md). |
...
| `honey` VR session | Design | `hello_xr` now reaches Monado runtime selection, Bigscreen Beyond selection, and Vulkan device creation on `honey`, but `xrCreateSession` still fails because `monado-service` crashes with `VK_ERROR_SURFACE_LOST_KHR`. This is a concrete substrate blocker, not a working VR smoke path yet. |
