...
| `yoga` XoxdWM install | Smoke | Public Rocky packages are installed on `yoga`; the compositor links cleanly and a bounded `seatd` runtime succeeds. Local session ergonomics remain follow-on work. |
...
| Desktop 2D compositor path | Smoke | Packaged and bounded on `yoga`, but not yet a polished local desktop/session lane. |
